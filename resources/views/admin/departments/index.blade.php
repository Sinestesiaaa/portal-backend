<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-6 py-10 space-y-6">
        ...
        <div class="bg-gradient-to-r from-[#0AA03A] to-[#16A34A] text-white p-6 rounded-xl shadow-md">
            <h1 class="text-2xl font-bold tracking-wide">🏢 Manajemen Departemen</h1>
            <p class="text-white/90 text-sm">Tambah, ubah, dan hapus departemen</p>
        </div>
        ...
        @if (session('error'))
            <div class="p-3 bg-red-200 text-red-900 rounded-lg shadow-sm">
                {{ session('error') }}
            </div>
        @endif
                ...
                <tbody>
                    @forelse ($departments as $dept)
                        <tr class="border-b hover:bg-[#F3FAF6]">
                            <td class="p-3 font-semibold text-gray-800">{{ $dept->name }}</td>
                            <td class="p-3 text-gray-700">{{ $dept->description ?? '-' }}</td>
