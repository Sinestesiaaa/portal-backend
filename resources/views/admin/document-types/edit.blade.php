<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto px-6 py-10 space-y-6">
        ...
        <div class="bg-gradient-to-r from-[#0AA03A] to-[#16A34A] text-white p-6 rounded-xl shadow-md">
            <h1 class="text-2xl font-bold tracking-wide">✏️ Edit Tipe Dokumen</h1>
            <p class="text-white/90 text-sm">Perbarui informasi tipe dokumen</p>
        </div>

        <div class="bg-white p-6 rounded-xl shadow-md border">
            <form action="{{ route('admin.document-types.update', $type->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="mt-1 w-full border rounded-lg px-4 py-2">{{ old('description', $type->description) }}</textarea>
                </div>

                <div>
                    <label class="font-semibold text-gray-700">Urutan</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $type->sort_order) }}"
